...
# Page 3: Model Training
    ...
    if st.session_state.processed_data is None:
        ...
        # Make sure the target column "Churn" exists in the data
        if 'Churn' not in processed_data.columns:
            st.error("Target variable 'Churn' not found in processed data.")
        else:
            # Separate features (X) from target (y)
            X = processed_data.drop('Churn', axis=1)   # All columns except Churn
            y = processed_data['Churn']               # Target column
            ...
            col1, col2 = st.columns(2)
            with col1:
                # Select what percentage of data goes into test set
            ...
            col1, col2, col3, col4 = st.columns(4)
            with col1:
                st.metric("Training Samples", f"{X_train.shape[0]:,}")   # Number of training rows
            with col2:
                st.metric("Testing Samples", f"{X_test.shape[0]:,}")    # Number of testing rows
            with col3:
                st.metric("Features", X_train.shape[1])                 # Number of features
            with col4:
                st.metric("Train Churn Rate", f"{y_train.mean():.2%}")  # Churn percentage in train set
            ...
            # User chooses which models to train
            ...
            with st.expander("⚙️ Model Hyperparameters", expanded=False):
                if "Neural Network (MLP)" in models_to_train:
                    ...
                    mlp_hidden_size = st.slider("Hidden Layer Size", 50, 300, 100)  # neurons in hidden layer
                    mlp_max_iter = st.slider("Max Iterations", 200, 1000, 500)     # training iterations
            
            # -------------------------------
            # TRAIN MODELS
            # -------------------------------
            if st.button("Train Models", type="primary"):
                models = {}  # Dictionary to store trained models and results
                
                with st.spinner("Training models..."):  # Loading animation
                    progress_bar = st.progress(0)       # Progress bar for model training
                    ...
                    if "Logistic Regression" in models_to_train:
                        ...
                        lr_pred_proba = lr_model.predict_proba(X_test)[:, 1]  # Probabilities for ROC-AUC
                    ...
                    if "Neural Network (MLP)" in models_to_train:
                        ...
                        mlp_model = MLPClassifier(
                            hidden_layer_sizes=(mlp_hidden_size,),  # Hidden layer size from user input
                            max_iter=mlp_max_iter,                 # Max iterations from user input
                            random_state=random_state,
                            early_stopping=True,                  # Stops early if no improvement
                            validation_fraction=0.1               # 10% of data for validation
                        )
                        mlp_model.fit(X_train, y_train)  # Train model
                        ...
                        progress_bar.progress(1.0)  # Training complete
                ...
                if models:
                    ...
                    styled_df = results_df.style.highlight_max(
                        axis=0, subset=['Accuracy', 'Precision', 'Recall', 'F1-Score', 'ROC-AUC']
                    )
